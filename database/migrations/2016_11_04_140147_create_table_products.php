...
class CreateTableProducts extends Migration
{
    ...
    public function up()
    {
        Schema::create('products', function (Blueprint $table) {
            ...
            $table->string('sku',32)->comment('物料号')->unique()->nullable();
            $table->string('model',32)->comment('物料型号')->nullable();
            $table->string('desc',128)->comment('产品描述')->nullable();
            $table->boolean('license')->comment('销售许可')->nullable();
            $table->boolean('approved')->nullable()->comment('注册信息批准');
            ...
            $table->timestamp('certified_at')->nullable()->comment('产品认证有效期');
            $table->timestamps();
            $table->softDeletes();
        });
    }
